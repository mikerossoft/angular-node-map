...
function treeBoxes(urlService, jsonData) {
        ...
        var linkenter = link
            ...
            .on('mouseover', function (d) {
                ...
                $('#tooltipLinkID' + d.target.id).attr(
                    'x',
                    (d.target.y + rectNode.width - d.source.y) / 2 + d.source.y
                );
                $('#tooltipLinkID' + d.target.id).attr(
                    'y',
                ...
                $('#tooltipLinkTextID' + d.target.id).css(
                    'visibility',
                    'visible'
                );
            })
        ...
        if (siblings) {
            for (var i = 0; i < siblings.length - 1; i++) {
                if (
                    ...
                    siblings[i + 1].x =
                        siblings[i].x + rectNode.height + minPadding;
            }
        }
    }
    ...
    function getMouseWheelEvent() {
        ...
        if (
            d3.select('#tree-container').select('svg').on('mousewheel.zoom') !=
            null
        ) {
